...
int verify_colision(Coordenate tab[][QTD_COLS_BLOCK], Coordenate peace[][TAM], int a, int b, int offset_side);
...
int main()
{
	...
	while (jhi_get_close_window() != JHI_CLOSE)
	{
		...
		for (i = 0; i < jhi_get_number_of_events(); i++){
			...
			colision = verify_colision(tab,peace,a,b,0);
			...
			if(colision){
				if(key.key == KEY_RIGHT){
					...
					colision = verify_colision(tab,peace,a,b,offset_side);
					
					if(colision){
						printf("%s\n","Colision RIGHT and BOTTOM" );
						offset_up = -1;
						colision_bottom = 1;
					}
					...
					if(colision){
						printf("%s\n","Colision LEFT and BOTTOM" );
						offset_up = -1;
						colision_bottom = 1;
					}
				}else if(key.key == KEY_UP){
					printf("%s\n","Colision ROTATION" );
					chose_peace(op,peace,rotation-1);
				}
				else{
					printf("%s\n","Colision BOTTOM" );
					offset_side = 0;
...
int verify_colision(Coordenate tab[][QTD_COLS_BLOCK], Coordenate peace[][TAM], int a, int b, int offset_side){
	...
	for(i=0;i<TAM;i++){
		for(j=0;j<TAM;j++){
			if(peace[i][j].fill){
				if(tab[a+i][b+j+offset_side].fill){
					colision = 1;
